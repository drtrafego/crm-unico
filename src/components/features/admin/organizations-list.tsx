...
function CopyableId({ id }: { id: string }) {
    ...
    return (
        <button onClick={handleCopy} className="flex items-center gap-1.5 group font-mono text-xs text-slate-500 hover:text-slate-700 dark:hover:text-slate-300 transition-colors">
        ...
        <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                {[
                    ...
                    <div key={card.label} className="rounded-2xl border border-slate-200 dark:border-white/8 bg-white dark:bg-white/3 p-5 flex items-center gap-4 backdrop-blur-sm shadow-sm">
            ...
            {/* List Container */}
            <div className="rounded-2xl border border-slate-200 dark:border-white/8 bg-white dark:bg-white/3 backdrop-blur-sm overflow-hidden shadow-sm">
                {/* Header */}
                <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between px-5 py-4 gap-3 border-b border-slate-200 dark:border-white/8">
                    <h2 className="text-base font-bold text-slate-900 dark:text-white">Organizações</h2>
                    <div className="relative w-full sm:w-60">
                        ...
                        <input
                            ...
                            className="w-full pl-9 pr-3 py-2 bg-slate-50 dark:bg-white/5 border border-slate-200 dark:border-white/10 rounded-xl text-sm text-slate-900 dark:text-white placeholder:text-slate-400 dark:placeholder:text-slate-500 focus:outline-none focus:border-indigo-500 transition-colors"
                        />
                    </div>
                </div>
                ...
                <div className="divide-y divide-slate-100 dark:divide-white/5">
                    ...
                    {filtered.map((org) => (
                        <div
                            key={org.id}
                            className="flex flex-col sm:flex-row items-start sm:items-center gap-4 px-5 py-4 hover:bg-slate-50 dark:hover:bg-white/3 transition-colors group"
                        >
                            {/* Avatar + Name */}
                            ...
                            {/* Stats */}
                            <div className="flex items-center gap-5 text-sm text-slate-500 dark:text-slate-400 shrink-0">
                                <div className="flex items-center gap-1.5">
                                    <Users className="h-3.5 w-3.5 text-slate-400 dark:text-slate-600" />
                                    <span className="text-slate-900 dark:text-white font-semibold">{org.totalLeads}</span>
                            ...
                            <div className="flex items-center gap-2 shrink-0">
                                <button
                                    ...
                                    className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-semibold text-slate-500 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white bg-slate-50 dark:bg-white/5 hover:bg-slate-100 dark:hover:bg-white/10 border border-slate-200 dark:border-white/8 hover:border-slate-300 dark:hover:border-white/15 rounded-xl transition-all"
                                >
                ...
                <div className="px-5 py-3 border-t border-slate-100 dark:border-white/5">
                    <span className="text-xs text-slate-500 dark:text-slate-600">{filtered.length} de {organizations.length} organização(ões)</span>
                </div>
            </div>
